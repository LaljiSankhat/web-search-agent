...
from services.llms import map_llm, deep_llm, response, sentiment_llm, sentiment_prompt
...
async def classify_text(state: AgentState):
    response = await sentiment_llm.ainvoke(
        sentiment_prompt.format_messages(
            user_text=state['userResponse']
        )
    )

    return "yes" if response.content == "positive" else "no"
